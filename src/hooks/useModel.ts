...
export function useModel(activeSection: string) {
  const [modelId, setModelId] = useState<string>(
    MODEL_PRESETS.find((p) => p.id === "llama32-1b-general")?.id ?? 
  ...
  const lastModelBySectionRef = useRef<Record<string, string>>({
    chat: MODEL_PRESETS.find((p) => p.id === "llama32-1b-general")?.id ?? MODEL_PRESETS.find((p) => p.kind === "webllm")?.id ?? MODEL_PRESETS[0].id,
