...
export function Sidebar({ theme, onThemeChange, mobileOpen, onMobileOpenChange }: SidebarProps) {
  ...
  const nav = (
    <nav className="flex flex-col gap-2 flex-1 min-h-0 relative justify-evenly md:w-auto md:min-w-0 md:flex-1 md:flex-row md:items-center md:justify-center md:gap-1">
      {APP_SECTIONS.map(({ id, label, path, Icon }) => {
        ...
        return (
          <div key={id} className="relative group/item isolate">
            ...
            <Link href={path} prefetch={true} onClick={closeMobile} className={`flex items-center gap-2 w-full h-14 py-0 px-3 rounded-xl transition-colors ${isActive ? "text-violet-600 dark:text-violet-400 bg-white/10" : "text-gray-400 dark:text-gray-300 hover:bg-white/10 dark:hover:bg-white/10"} md:w-auto md:h-11 md:px-3 md:rounded-xl`}>
              <span className={`flex items-center justify-center flex-shrink-0 w-6 h-6 overflow-hidden ${isActive ? "scale-110" : "transition-transform duration-200 group-hover/item:scale-110"}`}>
                <Icon size={18} className={isActive ? "text-white" : "text-gray-400 dark:text-gray-300 group-hover/item:text-white"} />
              </span>
              <span className="text-sm font-medium text-gray-300 dark:text-gray-200 truncate hidden md:inline">{label}</span>
            </Link>
            <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 px-2.5 py-1.5 text-xs font-medium rounded-lg bg-gray-800 text-white shadow-lg border border-gray-700/50 pointer-events-none z-[200] opacity-0 group-hover/item:opacity-100 transition-opacity duration-150 whitespace-nowrap md:hidden">{label}</span>
          </div>
        );
      })}
      <div className="relative group/item isolate">
        <a
          ...
          onClick={closeMobile}
          className="flex items-center gap-2 w-full h-14 py-0 px-3 rounded-xl transition-colors text-gray-400 dark:text-gray-300 hover:bg-white/10 dark:hover:bg-white/10 md:w-auto md:h-11 md:px-3 md:rounded-xl"
          aria-label="Open WebGPU Studio on GitHub"
        >
          <span className="flex items-center justify-center flex-shrink-0 w-6 h-6 overflow-hidden transition-transform duration-200 group-hover/item:scale-110">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden className="text-gray-400 dark:text-gray-300 group-hover/item:text-white">
          ...
          <span className="text-sm font-medium text-gray-300 dark:text-gray-200 truncate hidden md:inline">GitHub</span>
        </a>
        <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 px-2.5 py-1.5 text-xs font-medium rounded-lg bg-gray-800 text-white shadow-lg border border-gray-700/50 pointer-events-none z-[200] opacity-0 group-hover/item:opacity-100 transition-opacity duration-150 whitespace-nowrap md:hidden">GitHub</span>
      </div>
    </nav>
  );

  const themeBlock = (
    <div className="themeDivider flex-shrink-0 mt-6 pt-4 border-t border-gray-200/50 dark:border-gray-700/50 flex justify-start px-2 md:mt-0 md:pt-0 md:pl-3 md:pr-1 md:ml-2 md:border-t-0 md:border-l md:border-white/10 md:flex-shrink-0 md:relative md:z-10">
      <ThemeToggle theme={theme} onThemeChange={onThemeChange} className="w-9 h-9 shrink-0 text-gray-300 border-white/15 bg-white/5" />
    </div>
  );
  ...
  */
  ...
  return (
    <>
      ...
      <aside
        className={`sidebarDock group fixed left-0 top-0 bottom-0 z-50 w-[72px] py-5 px-2 flex flex-col bg-white/20 dark:bg-black/20 backdrop-blur-2xl border-r border-gray-200/50 dark:border-gray-700/50 shadow-xl transition-[transform,width] duration-200 ease-out
          ${mobileOpen ? "translate-x-0" : "-translate-x-full"}
          md:left-1/2 md:bottom-6 md:top-auto md:h-[62px] md:-translate-x-1/2 md:translate-y-0 md:flex-row md:items-center md:overflow-visible md:rounded-full md:border md:border-white/10 md:bg-black/90 md:px-3 md:w-[min(64vw,1280px)] md:max-w-[calc(100vw-32px)]`}
